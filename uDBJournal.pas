...
uses
  u_DMDatabase, uJournalSpec, pFIBDatabase, uVehicles3C,
  FIBQuery, Variants, SysUtils, pFIBQuery;


type
  TJournDataWriter = class(TDBWriter)
  private
...
uses
  u_ByteOrders, uCommProtocol_3C, u_SQLs, u3SSensors, StrConvUnit;

{ TJournDataWriter }

constructor TJournDataWriter.Create(DB: TpFIBDatabase);
begin
  inherited;
  PrevSesID := 0;
  PrevJrnType := $FF;
...
procedure TJournDataWriter.FillRecHeader;
begin
...
function TJournDataWriter.ParseBuffer: Boolean;
var
  iPack, iRec: Integer;
  UndefRec: PByte;
begin
...
procedure TJournDataWriter.UpdateState;
var
  f: Byte;
  IsGPSFake: Boolean;
  qrState: TFIBQuery;
...
function TJournDataWriter.WriteData(AData: PByte; AVid: TVehIdent): Boolean;
var
  CurrSesID: Word;
begin
  Vid := AVid;
  ...
  if ((PrevJrnType <> CurrentPacket.JrnType) or (PrevSesID <> CurrSesID)) and (SessionData.Count > 0) then Result := False
  else
  begin
...
procedure TJournDataWriter.WriteEventRec(RecData: PByte);
var
  Struct: PEventJrn;
  s: String;
begin
...
procedure TJournDataWriter.WriteExtSensRec(RecData: PByte);
var
  Struct: PExtSensJrn;
  s: String;
  CC, CC1: Byte;
...
procedure TJournDataWriter.WriteGPSRec(RecData: PByte);
var
  Struct: PGPSJrn;
begin
...
procedure TJournDataWriter.WriteSensRec(RecData: PByte);
var
  Struct: PSensJrn;
  CC: Byte;
...
procedure TJournDataWriter.WriteStateRec(RecData: PByte);
var
  Struct: PStateJrn;
begin
